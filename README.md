...
[问财（iwencai.com）](https://www.iwencai.com/) 是同花顺（Hithink RoyalFlush）旗下的 AI 选股平台，支持用中文自然语言描述筛选条件（如"市盈率小于20"、"连续三天涨停"、"日线 MACD 金叉"等），后端自动解析为量化条件。本工具把问财的网页查询能力封装为本地 CLI，便于脚本化、批量化使用。
